    ...
    <v-card v-if="order">
      ...
      <v-card-text>
        <v-alert type="info" variant="tonal" class="mb-4" density="compact">
          ...
          <div class="text-caption">FIO Quantity: {{ order.fioQuantity.toLocaleString() }}</div>
        </v-alert>
...
import { api, type SellOrderResponse, type EffectivePrice } from '../services/api'
...
const save = async () => {
  ...
  try {
    saving.value = true
    await api.sellOrders.update(props.order.id, {
