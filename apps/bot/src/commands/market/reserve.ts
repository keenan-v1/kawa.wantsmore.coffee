...
export const reserve: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const selectMenu = new StringSelectMenuBuilder()
      ...
      .setPlaceholder('Select an order to reserve from')
      .setMinValues(1)
      .setMaxValues(1)
      .addOptions(options)
    ...
    try {
      ...
      const modal = new ModalBuilder()
        .setCustomId(modalId)
        .setTitle(`Reserve ${selectedOrder.commodityTicker}`)

      const quantityInput = new TextInputBuilder()
        .setCustomId('quantity')
        .setLabel(`Quantity (max ${selectedOrder.quantity} available)`)
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `You have reserved **${quantity}x ${formatCommodity(order.commodityTicker)}** ` +
          ...
          `**Total:** ${totalCost} ${displayCurrency}\n\n` +
